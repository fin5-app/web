...
function NavbarItem(props: NavbarItemProps) {
  ...
  if (variant === 'button') {
    return (
      <Link
        to={href}
        className="py-2 px-3 text-primary-100 bg-text-primary rounded-[6px] font-medium text-sm"
      >
        <p>{name}</p>
      </Link>
    )
  ...
  return (
    <Link
      to={href}
      className="py-2 px-3 text-text-primary rounded-[6px] font-medium text-sm"
    >
      <p>{name}</p>
    </Link>
  )

...
export const CardsWrapper = (props: CardsWrapperProps) => {
    ...
    return (
        <div className="w-full grid grid-cols-1 xl:grid-cols-3 gap-5">
            <CardBalance title="Ingresos" amount={data?.total_incomes} type="incomes" />
            <CardBalance title="Gastos" amount={data?.total_expenses} type="expenses" />
            <CardBalance title="Porcentaje de Ahorro" amount={((data?.total_saving ?? 0) * 100) / (data?.total_incomes ?? 0)} type="savings" />
        </div>
    )
}
...
function CardBalance(props: CardBalanceProps) {
    ...
    return (
        <div className='bg-secondary-100 py-3 px-5 rounded-[6px] flex items-center border-[1px] border-borderPrimary-100 space-x-4'>
            <div>
                {
                    Icon ? <Icon className='h-10 w-10 text-purple-100' /> : null
                }
            </div>
            <div>
                <p className='text-text-primary'>{title}</p>
                {
                    type === 'savings' ?
                        ...
                        <p className={`font-semibold  text-xl xl:text-sm ${TextColor}`}>%{Math.ceil(amount ?? 0)}</p>
                        :
                        <p className={`font-semibold  text-xl xl:text-sm ${TextColor}`}>{formatCurrency(amount ?? 0, 'ARS')}</p>
                }
            </div>
        </div>
    )

...
export const ForgotPasswordForm: FC = () => {
  const { handleNavigateToLoginPage, handleBlur, handleChange, handleSubmit, values} = useForgotPasswordController();
  return (
    <div className='bg-secondary-100 border-borderPrimary-100 border-[1px] p-4 rounded-md space-y-4 w-[320px]'>
      <div className="space-y-2">
        <span className="text-white text-normal text-sm ">Introduce tu dirección de correo electrónico asociada a tu cuenta para restablecer tu contraseña</span>
      </div>
      <div className='space-y-2'>
        <label htmlFor="email" className='text-white text-normal text-sm'>Dirección de correo electrónico</label>
        <input

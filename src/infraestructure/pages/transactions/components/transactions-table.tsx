...
export const TransactionsTable: FC = () => {
    ...
    const { data: transactions } = useGetTransactions(Number(params.get('page')), Number(params.get('items')), params.get('q') ?? undefined);
    return (
        <div className='w-full bg-secondary-100 border-[1px] border-borderPrimary-100 min-h-[%50] rounded-[6px]'>
            ...
            <TableFooter total_pages={transactions?.total_pages} current_page={Number(params.get('page'))} />
        </div>
    )
}
...
const TableRow: FC<TableRowProps> = (props) => {
    return (
        <div className="grid grid-cols-6 px-3 py-4 border-b-[1px] border-borderPrimary-100 items-center hover:cursor-pointer">
            <div className="col-span-2">
                <p className="text-text-primary text-sm font-light">{props.category.name}</p>
            </div>
            <div>
                <p className="text-text-primary text-sm font-light">{new Date(props.created).toLocaleDateString('es-AR', { day: '2-digit', month: '2-digit', year: 'numeric' })}</p>
            </div>
            <div>
                <p className={`uppercase w-fit text-secondary-100 px-2  text-[0.65rem] font-bold rounded-full ${props.type.name === TransactionTypes.EXPENSE ? 'bg-error-100' : 'bg-success-100'}`}>
                    {
            ...
            <div>
                <p className="text-text-primary text-sm font-light">{formatCurrency(props.amount, 'USD')}</p>
            </div>
            <div>
                <p className="text-text-primary text-sm font-light">{props.currency.code}</p>
            </div>
